...
import Network.Transport.TCP (TCPAddr (..), TCPAddrInfo (..), createTransport, defaultTCPParameters)
...
createLocalNode host port mkExternal rTable = do
  transport <-
    either (error . show) id
      <$> createTransport (Addressable $ TCPAddrInfo {tcpBindHost = host, tcpBindPort = port, tcpExternalAddress = mkExternal}) defaultTCPParameters
...
peerController seeds = do
  ...
  forever $
    receiveWait
      [ matchIf isPeerDiscover $ onDiscover state,
...
onDiscover _ (WhereIsReply _ (Just seedPid)) = do
...
getPeers = do
  ...
  receiveChan rp <&> map processNodeId . S.toList
...
getCapable service = do
  ...
  where
    go rp acc = do
      res <- receiveChanTimeout 100000 rp
      case res of
        Just pid -> say "cap hit" >> go rp (pid : acc)
